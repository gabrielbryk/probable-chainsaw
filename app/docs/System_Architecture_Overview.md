...
| Bridges | Translate backend events into smart-home/device actions the browser cannot perform. | Node bridge for Govee/Google Home, webhook clients |
...
| **Google Home / IFTTT / Nabu Casa** | Kicks off broader smart-home scenes (“Hey Google, celebrate Joy!”). | Bridge posts webhook payloads containing `{event, payload}` to automation endpoints. | `GOOGLE_HOME_WEBHOOK_URL` (or similar). |
...
| **SQLite via Prisma** | Persists riddles, attempts, hint logs, and effect history. | Backend uses Prisma client generated from `schema.prisma`. | `DATABASE_URL` (defaults to local SQLite path). |
...
- The **Lighting Bridge** subscribes to events and calls the Govee API or Google Home routines.
...
- Required secrets: `OPENAI_API_KEY`, `GOVEE_API_KEY`, `GOOGLE_HOME_WEBHOOK_URL`, optional `SENTRY_DSN`.
...
- **Riddle Engine ↔ Lighting Bridge**: Webhook or lightweight message queue (e.g., SSE) that passes `{event, riddleId, payload}`.
